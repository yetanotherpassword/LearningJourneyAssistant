...
| A-29 | Report which LLM provider/model produced a clustering, on every run | T1 | — | **OPEN** | Raised 2026-08-26. `cli.py:96` prints `LLM: {client.describe()}` only inside the *else* of the cache check, so a cached run — which is every run once `output/silo_clustering.json` exists — never says which backend is configured. Two parts: (a) print `config.LLM_PROVIDER` unconditionally at startup, **from config, not by building a client** — constructing one the cache path never uses could fail on a missing API key and break a run that needed no LLM at all; (b) record provider and model *inside* `silo_clustering.json` when it is written, so a cached run reports which model actually produced the cache rather than which is configured now. Those differ, and nothing currently tells you — which matters given the README's finding that an 8B model produced semantically useless clusters where the 30B one worked. Land it on `s3-6/relative-gap-detection`; `cli.py` is already modified there. |
| A-30 | Make LLM failures diagnosable — report a timeout as a timeout, and set an explicit configurable timeout | T1 | — | **OPEN** | Raised 2026-08-26 after a real 30-minute failure that reported the wrong cause. Two defects in `lja/llm/openai_compatible_client.py`. **(a)** A timeout is reported as a JSON-shape problem: the handler catches `APITimeoutError` in the same net as malformed responses and raises "never returned valid SiloClusteringResult JSON after trying 3 response_format strategies". The actual cause was `httpx.ReadTimeout` — no reply at all. `finish_reason=None` and `Last raw reply: None` were the only tells, and they read as incidental. A timeout and a bad response need different reactions from the user (check the server vs change the prompt or model), so they must not share an error message. **(b)** Line 55 constructs `OpenAI(base_url=..., api_key=...)` with **no `timeout`**, so it silently inherits the SDK's 600 s default — and because the client retries three `response_format` strategies, a hung local server burns **30 minutes** before failing. Add an `LJA_OPENAI_TIMEOUT` config value with a sane default, and say which timeout was hit when it fires. **Worth doing for the demo:** Anup runs the sprint review from a clean clone, and if his GPU is busy this is exactly the failure he will hit, with an error message pointing at the wrong thing. |
...
| A-23 | Stop hardcoding the test count in the READMEs | Allan | C3 | **DONE 2026-09-06** — the number was removed from the root README (PR #6) and `python/README.md` (PR #5) rather than corrected a fifth time. CI is the source of truth. |
